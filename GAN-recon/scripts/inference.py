...
def main(args):
    ...
    if not args.latents_only:
        generate_inversions(args, generator, latent_codes, is_cars, data_loader)
...
def save_image(img, save_dir, filename):
    ...
    im_save_path = os.path.join(save_dir, filename)
    Image.fromarray(np.array(result)).save(im_save_path)


@torch.no_grad()
def generate_inversions(args, g, latent_codes, is_cars, data_loader):
    print('正在保存逆向图像')
    ...
    for i in range(min(args.n_sample, len(latent_codes))):
        ...
        # 从数据集中获取文件名
        filename = os.path.basename(dataset.img_paths[i])
        save_image(imgs[0], inversions_directory_path, filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images_dir", type=str, default="D:\pythonProject_diffusion\REAL/car_512",
                        help="The directory of the images to be inverted")
    parser.add_argument("--save_dir", type=str, default="D:\GAn\encoder4editing-main/1",
