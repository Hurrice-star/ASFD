...
def get_network(arch: str, isTrain=False, continue_train=False, init_gain=0.02, pretrained=True, num_classes=3):
    if "resnet" in arch:
        ...
        if isTrain:
            if continue_train:
                model: ResNet = resnet(num_classes=num_classes)  # 使用 num_classes
            else:
                model: ResNet = resnet(pretrained=pretrained)
                model.fc = nn.Linear(6144, num_classes)  # 调整输入特征到 4096 以适应三输入
