...
class Trainer(BaseModel):
    ...
    def __init__(self, cfg: CONFIGCLASS):
        ...
        self.model = get_network(self.arch, cfg.isTrain, cfg.continue_train, cfg.init_gain, cfg.pretrained, num_classes=self.num_classes)
    ...
    def set_input(self, input_data):
        inputs1, inputs2, inputs3, labels = input_data
        self.inputs1 = inputs1.to(self.device)
        self.inputs2 = inputs2.to(self.device)
        self.inputs3 = inputs3.to(self.device)
        self.labels = labels.to(self.device)

    def forward(self):
        # Forward pass with three inputs
        self.output = self.model(self.inputs1, self.inputs2, self.inputs3)

    def get_loss(self):
        # Calculate the loss
        return self.loss_fn(self.output, self.labels)
