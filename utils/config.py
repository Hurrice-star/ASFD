...
class DefaultConfigs(ABC):
    ...
    gpus = [0]
    seed = 3407
    arch = "resnet50_three_input"  # 修改为支持双输入图像的ResNet50架构
    datasets = ["dataset"]  # 训练数据集
    datasets_test = ["2"]  # 测试数据集
    mode = "binary"  # 可以根据需求修改为 "filename" 模式等
    class_bal = False  # 是否启用类平衡
    batch_size = 32
    loadSize = 256
    cropSize = 224
    epoch = "latest"
    ...
    dataset_root = os.path.join(root_dir, "data")  # 数据集根目录

    
    exp_root = os.path.join(root_dir, "data", "exp")  # 实验目录
    _exp_name = ""
    exp_dir = ""
    ckpt_dir = ""
    logs_path = ""
...
parser.add_argument("--ckpt", default="model_epoch_best.pth", type=str)
